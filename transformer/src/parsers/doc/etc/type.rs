...
fn parse_type_wth_parent_test() {
    ...
    let c = Type::try_from((&xmltree::XMLNode::Element(tree), &xmltree::XMLNode::Element(tree.clone()), "test")).unwrap();
...
fn parse_type_that_is_attribute_test() {
    ...
    "#;
    let tree = xmltree::Element::parse(xml).unwrap();
    let c = Type::try_from((&xmltree::XMLNode::Element(tree), &xmltree::XMLNode::Element(tree), "test")).unwrap();
    let value = openapiv3::Schema::from(&c);
    assert_eq!(
        serde_json::to_value(value).unwrap(),
        json!({
